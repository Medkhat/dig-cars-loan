...
import { useCoBorrowerCancelMutation } from '@/features/api/co-borrower-';
import { logout } from '@/features/auth/tasks';
import ConfirmationModal from '@/views/confirmation-modal';

const CoborrowerRejectCreditBlock = ({ flowUuid }) => {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);

  const dispatch = useDispatch();

  const [fetchRejectCredit] = useCoBorrowerCancelMutation();
...
export default CoborrowerRejectCreditBlock;
